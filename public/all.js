    ...
    $.ajax({
        ...
        success: (data) => {
            if(data.length !== 0){
                let write=[]
                for(let i=0;i<data.length;i++){
                    write+=`<div class="col-3" style="margin-bottom: 50px;">
                                ...
                                <div id="matter" class="text-center">
                                    <a href="/blog?title=${data[i].title}" class="btn btn-outline-dark btn-block"><h1 class="card-title">${data[i].title}</h1></a>
                                </div>
                                <div class="card-text text-center">
                                <b>Author:</b> ${data[i].author} <br> <b>Created:</b> ${data[i].date} <br> <b>👍:</b> <span>${data[i].likes}</span>
                                </div>
                                </div>
                                </div>
                            </div>`
        ...
        $.ajax({
            ...
            success: (data) => {
                if(data.length !== 0){
                    let writep=[]
                    for(let i=0;i<data.length;i++){
                        writep+=`<div class="col-3" style="margin-bottom: 50px;">
                                    <div class="card border-dark">
                                    <img src="${data[i].coverimg}" class="card-img-top" alt="coverimage">
                                    <img src="${data[i].dp}" class="dp shadow" alt="profile img" width="75px">
                                    <h5 class="card-subtitle text-muted mb-2 text-center"><b>Category:</b> ${data[i].category}</h5>
